...
def parse_sweep_summary(summary_path):
    ...
    for line in lines:
        ...
        if line.startswith('Omega Sweep Summary') or 'Omega' in line and 'Sweep' in line:
        ...
        elif line.startswith('Gamma_imag Sweep Summary') or ('Gamma_imag' in line and 'Sweep' in line):
            sweep_type = 'gamma_imag'
            metadata['sweep_type'] = 'gamma_imag'
        elif line.startswith('Gamma_real Sweep Summary') or ('Gamma_real' in line and 'Sweep' in line):
            sweep_type = 'gamma_real'
            metadata['sweep_type'] = 'gamma_real'
            ...
            if sweep_type == 'lambda':
                ...
                metadata['gamma_imag_min'] = param_min
                metadata['gamma_imag_max'] = param_max
            elif sweep_type == 'gamma_real':
                metadata['gamma_real_min'] = param_min
                metadata['gamma_real_max'] = param_max
            else:
            ...
            if len(parts) >= 4:
                try:
                    ...
                    if sweep_type == 'lambda':
                        ...
                        result['gamma_imag'] = param_val
                    elif sweep_type == 'gamma_real':
                        result['gamma_real'] = param_val
                    else:
...
def find_sweep_summaries(directory='.'):
    patterns = [
        os.path.join(directory, 'results/imdb/sweep_omega_*_summary.txt'),
        os.path.join(directory, 'results/imdb/sweep_lambda_*_summary.txt'),
        os.path.join(directory, 'results/imdb/sweep_gamma_imag_*_summary.txt'),
        os.path.join(directory, 'results/imdb/sweep_gamma_real_*_summary.txt'),
        os.path.join(directory, 'results/imdb/sweep_*_summary.txt')
    ]
    summaries = set()
...
def plot_param_vs_accuracy(results, metadata, output_path):
    ...
    if sweep_type == 'lambda':
        ...
        param_name = 'gamma_imag'
        param_label = 'Gamma Imaginary ($\\gamma_i$)'
        param_values = [r['gamma_imag'] for r in results]
        param_min = metadata.get('gamma_imag_min', min(param_values))
        param_max = metadata.get('gamma_imag_max', max(param_values))
        title_prefix = 'Gamma Imaginary Sweep'
    elif sweep_type == 'gamma_real':
        param_name = 'gamma_real'
        param_label = 'Gamma Real ($\\gamma_r$)'
        param_values = [r['gamma_real'] for r in results]
        param_min = metadata.get('gamma_real_min', min(param_values))
        param_max = metadata.get('gamma_real_max', max(param_values))
        title_prefix = 'Gamma Real Sweep'
    else:
        param_name = 'omega'
    ...
    if sweep_type == 'lambda':
        ...
        param_symbol = '$\\gamma_i$'
    elif sweep_type == 'gamma_real':
        param_symbol = '$\\gamma_r$'
    else:
        param_symbol = '$\\omega$'
...
def plot_accuracy_comparison(results, metadata, output_path):
    ...
    if sweep_type == 'lambda':
        ...
        param_values = [r['gamma_imag'] for r in results]
        title_prefix = 'Gamma Imaginary Sweep'
        param_label = 'Gamma Imaginary'
    elif sweep_type == 'gamma_real':
        param_values = [r['gamma_real'] for r in results]
        title_prefix = 'Gamma Real Sweep'
        param_label = 'Gamma Real'
    else:
        param_values = [r['omega'] for r in results]
        title_prefix = 'Omega Sweep'
        param_label = 'Omega'
...
def plot_combined_overview(results, metadata, output_path):
    ...
    if sweep_type == 'lambda':
        ...
        param_values = [r['gamma_imag'] for r in results]
        param_label = 'Gamma Imaginary ($\\gamma_i$)'
        param_label_plain = 'Gamma Imaginary'
        param_min = metadata.get('gamma_imag_min', min(param_values))
        param_max = metadata.get('gamma_imag_max', max(param_values))
        title_prefix = 'Gamma Imaginary Sweep'
    elif sweep_type == 'gamma_real':
...
def analyze_sweep(summary_path, output_dir=None):
    ...
    if sweep_type == 'lambda':
        ...
        print(f"  Gamma Imaginary range: {metadata.get('gamma_imag_min', 'N/A'):.6f} to {metadata.get('gamma_imag_max', 'N/A'):.6f}")
    elif sweep_type == 'gamma_real':
        print(f"  Gamma Real range: {metadata.get('gamma_real_min', 'N/A'):.6f} to {metadata.get('gamma_real_max', 'N/A'):.6f}")
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Analyze omega sweep results')
    ...
    if args.summary:
        ...
        if not summaries:
            print("No sweep summary files found!")
            print("Usage: python imdb_sweep_analysis.py --all")
